...
def generiere_rueckblick(daten, pivots, tendenz, zonen_je_zeitraum, szenarien, langfrist_formation=None, mittelfristige_szenarien=None, intraday_zukunft=None, tages_ma_struktur=None):
    ...
    if chartkanal:
        chart_quantitative_block = (
            f"- Aktueller Kurs: {chartstruktur['kurs']:.2f} USD\n"
            ...
            f"({chartkanal['abstand_oben_pct']:+.2f}% vom aktuellen Kurs)\n"
            f"- Abstand zur unteren Begrenzung: {chartkanal['abstand_unten']:+.2f} USD "
            f"({chartkanal['abstand_unten_pct']:+.2f}% vom aktuellen Kurs)\n"
        )
...
Diese Chartstruktur stammt aus derselben bestehenden Berechnung wie die sichtbare Grafik. Für die Zukunftseinschätzung sind die numerischen Kanalgrenzen am aktuellen Kurszeitpunkt verbindlich:
{chart_quantitative_block}
