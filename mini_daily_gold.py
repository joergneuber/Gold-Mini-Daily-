...
def finde_lokale_reaktionszonen(intraday_reihe, stunden=36, bucket_usd=5, min_treffer=2,
                                  top_n=3, extrema_fenster=1, min_abstand_bars=3):
    """Ermittelt kurzfristige Reaktionszonen aus den letzten `stunden` Stunden.

    Anders als die ältere strenge Swing-Erkennung verlangt diese Variante keine
    2-Kerzen-Bestätigung links/rechts. Sie sucht lokale Hochs/Tiefs mit nur einem
    direkten Nachbarn je Seite, clustert sie in 5-USD-Bereiche und verlangt
    mindestens `min_treffer` zeitlich getrennte Reaktionen. Damit werden auch
    kurzfristige, im Intraday-Chart klar sichtbare Reaktionsniveaus wie ca. 4.360 USD
    erkannt, ohne einzelne benachbarte Kerzen als mehrere Treffer zu zählen.

    Die Funktion ist rein charttechnisch/rückblickend und wird nicht als
    zukunftsgerichtetes Handelssignal verwendet.
    """
    if intraday_reihe is None or intraday_reihe.empty:
        return {"widerstandszonen": [], "supportzonen": []}

    reihe = intraday_reihe.copy()
    letzter_zeitpunkt = reihe.index[-1]
    start = letzter_zeitpunkt - pd.Timedelta(hours=stunden)
    reihe = reihe.loc[reihe.index >= start]
    if len(reihe) < (2 * extrema_fenster + min_abstand_bars + 2):
        return {"widerstandszonen": [], "supportzonen": []}

    highs = reihe["High"].astype(float).to_numpy()
    lows = reihe["Low"].astype(float).to_numpy()
    index = reihe.index

    def lokale_extrema(werte, ist_hoch):
        punkte = []
        n = len(werte)
        for i in range(extrema_fenster, n - extrema_fenster):
            links = werte[i-extrema_fenster:i]
            rechts = werte[i+1:i+extrema_fenster+1]
            if ist_hoch:
                if (werte[i] >= links.max() and werte[i] >= rechts.max()
                        and (werte[i] > links.max() or werte[i] > rechts.max())):
                    punkte.append((i, float(werte[i])))
            else:
                if (werte[i] <= links.min() and werte[i] <= rechts.min()
                        and (werte[i] < links.min() or werte[i] < rechts.min())):
                    punkte.append((i, float(werte[i])))
        return punkte

    def clustern(punkte):
        buckets = {}
        for pos, preis in punkte:
            key = round(preis / bucket_usd) * bucket_usd
            buckets.setdefault(key, []).append((pos, preis))

        zonen = []
        for werte in buckets.values():
            # Mehrere unmittelbar aufeinanderfolgende Extrempunkte dürfen nicht
            # künstlich mehrere Treffer erzeugen. Es zählen nur zeitlich getrennte
            # Reaktionen innerhalb derselben Preiszone.
            werte = sorted(werte, key=lambda x: x[0])
            getrennt = []
            for pos, preis in werte:
                if not getrennt or pos - getrennt[-1][0] >= min_abstand_bars:
                    getrennt.append((pos, preis))

            if len(getrennt) >= min_treffer:
                zonen.append((sum(p for _, p in getrennt) / len(getrennt),
                              len(getrennt),
                              [index[pos] for pos, _ in getrennt]))

        zonen.sort(key=lambda z: (-z[1], abs(z[0] - float(reihe["Close"].iloc[-1]))))
        return [(float(preis), int(treffer), zeiten) for preis, treffer, zeiten in zonen[:top_n]]

    return {
        "widerstandszonen": clustern(lokale_extrema(highs, True)),
        "supportzonen": clustern(lokale_extrema(lows, False)),
    }
...
def baue_chart(intraday_reihe, pivots, strukturzonen=None, range_ausbruch_status=None, pfad="chart.png"):
    ...
    # Kurzfristige Reaktionszonen: bewusst auf die letzten 36 Stunden begrenzt.
    # Diese Erkennung ist weniger streng als die ältere 2+2-Swing-Logik und zählt
    # mindestens zwei zeitlich getrennte Reaktionen innerhalb eines 5-USD-Buckets.
    lokale_reaktionszonen = finde_lokale_reaktionszonen(
        intraday_reihe, stunden=36, bucket_usd=5, min_treffer=2, top_n=3,
        extrema_fenster=1, min_abstand_bars=3
    )
    alle_umkehr_preise = (
        [p for p, _, _ in lokale_reaktionszonen["widerstandszonen"]]
        + [p for p, _, _ in lokale_reaktionszonen["supportzonen"]]
    )
    ...
    # Lokale 36h-Reaktionszonen zeichnen. Zonen innerhalb einer bereits gezeichneten
    # Range-Box werden übersprungen, damit der Chart nicht doppelt beschriftet wird.
    def in_box(p):
        return box_bereich is not None and box_bereich[0] <= p <= box_bereich[1]

    for preis, treffer, _ in lokale_reaktionszonen["widerstandszonen"]:
        if y_unten <= preis <= y_oben and not in_box(preis):
            ax.axhline(preis, color="#6fa8dc", linewidth=1.1, linestyle="-", alpha=0.75)
            ax.text(intraday_reihe.index[-1], preis,
                    f"  Reaktionszone {preis:,.0f} ({treffer}x)".replace(",", "."),
                    color="#6fa8dc", fontsize=7.5, va="bottom", ha="right")
    for preis, treffer, _ in lokale_reaktionszonen["supportzonen"]:
        if y_unten <= preis <= y_oben and not in_box(preis):
            ax.axhline(preis, color="#6fa8dc", linewidth=1.1, linestyle="-", alpha=0.75)
            ax.text(intraday_reihe.index[-1], preis,
                    f"  Reaktionszone {preis:,.0f} ({treffer}x)".replace(",", "."),
...
def main():
    ...
    lokale_reaktionszonen = finde_lokale_reaktionszonen(
        daten["intraday_reihe"], stunden=36, bucket_usd=5, min_treffer=2, top_n=3,
        extrema_fenster=1, min_abstand_bars=3
    )
    print("Lokale Reaktionszonen (36h):")
    print(f"  Widerstand: {[(round(p, 2), t) for p, t, _ in lokale_reaktionszonen['widerstandszonen']]}")
    print(f"  Support: {[(round(p, 2), t) for p, t, _ in lokale_reaktionszonen['supportzonen']]}")
