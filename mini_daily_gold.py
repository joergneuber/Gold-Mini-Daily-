...
def analysiere_intraday_zukunft(daten, szenarien, intraday_chartstruktur=None):
    """1h Richtung, 30m Setup, 15m kurzfristige Entwicklung; nur konditionale Szenarien.
    Die bereits für den Intraday-Chart berechnete Kanal-/Trendstruktur kann optional
    übernommen werden, damit die Zukunftseinschätzung denselben Chartkontext verwendet.
    """
    frames = {
    ...
    return {
        ...
        "chartstruktur": intraday_chartstruktur,
    }
...
def formatiere_intraday_zukunft(zukunft, fmt):
    ...
    chart = zukunft.get("chartstruktur") or {}
    chart_zeile = chart.get("text") if chart else None
    zeilen = [
        f"INTRADAY-ZUKUNFTSANALYSE | Intraday-Bild: {zukunft['bias']} | Erwartung: {zukunft['setup']}",
        zeile("1h", "übergeordnete Richtung", f["1h"]),
        zeile("30m", "aktuelles Setup", f["30m"]),
        zeile("15m", "kurzfristige Entwicklung", f["15m"]),
    ]
    if chart_zeile:
        zeilen.append(f"Chartstruktur: {chart_zeile}")
        chart_zeitpunkt = chart.get("kurs_zeitpunkt")
        if chart_zeitpunkt is not None:
            zeilen.append(
                f"Kanalbezug: 1h-Schlusskurs {fmt(chart.get('kurs', f['1h']['close']))} USD "
                f"am {pd.Timestamp(chart_zeitpunkt).strftime('%d.%m.%Y %H:%M UTC')}"
            )
    zeilen.extend([
...
def generiere_rueckblick(daten, pivots, tendenz, zonen_je_zeitraum, szenarien, langfrist_formation=None, mittelfristige_szenarien=None, intraday_zukunft=None, tages_ma_struktur=None):
    ...
    chartstruktur = intraday_zukunft.get("chartstruktur") if intraday_zukunft else None
    chartkanal = chartstruktur.get("kanal") if chartstruktur else None
    if chartkanal:
        kanal_zeitpunkt = chartkanal.get("kurs_zeitpunkt") or chartstruktur.get("kurs_zeitpunkt")
        kanal_zeitpunkt_text = (
            pd.Timestamp(kanal_zeitpunkt).strftime("%d.%m.%Y %H:%M UTC")
            if kanal_zeitpunkt is not None else "Zeitpunkt nicht verfügbar"
        )
        chart_quantitative_block = (
            f"- Kanalbezugs-Kurs (1h-Schlusskurs): {chartkanal['kurs']:.2f} USD\n"
            f"- Gemeinsamer Berechnungszeitpunkt von Kurs und Kanalgrenzen: {kanal_zeitpunkt_text}\n"
            f"- Obere Kanalbegrenzung aktuell: {chartkanal['obere_grenze']:.2f} USD\n"
            f"- Untere Kanalbegrenzung aktuell: {chartkanal['untere_grenze']:.2f} USD\n"
            f"- Kanalbreite: {chartkanal['kanalbreite']:.2f} USD\n"
            f"- Abstand zur oberen Begrenzung: {chartkanal['abstand_oben']:+.2f} USD "
            f"({chartkanal['abstand_oben_pct']:+.2f}% vom Kanalbezugs-Kurs)\n"
            f"- Abstand zur unteren Begrenzung: {chartkanal['abstand_unten']:+.2f} USD "
            f"({chartkanal['abstand_unten_pct']:+.2f}% vom Kanalbezugs-Kurs)\n"
        )
        if chartkanal.get("position_im_kanal_pct") is not None:
            position_pct = chartkanal["position_im_kanal_pct"]
            if 0.0 <= position_pct <= 100.0:
                position_block = f"- Position im Kanal: {position_pct:.1f}% von unten nach oben\n"
            elif position_pct > 100.0:
                position_block = f"- Position relativ zum Kanal: {position_pct:.1f}% (oberhalb)\n"
            else:
                position_block = f"- Position relativ zum Kanal: {position_pct:.1f}% (unterhalb)\n"
            chart_quantitative_block += position_block
    else:
        chart_quantitative_block = "- Keine numerischen Kanalgrenzen verfügbar.\n"

    prompt = f"""Du bist ein nüchterner charttechnischer Kommentator für Gold Spot (XAU/USD).

Deine Aufgabe ist NICHT, die bereits berechnete Analyse zu wiederholen, sondern daraus eine kurze ZUKUNFTSEINSCHÄTZUNG abzuleiten: Welche kurzfristige Entwicklung wird durch die aktuelle Datenlage unterstützt, welche bereits berechneten Bedingungen bestätigen sie und was würde sie widerlegen? Stelle außerdem den Bezug zur übergeordneten 6M-/Positionsstruktur her.

REGELN:
- Verwende ausschließlich die vom Programm gelieferten Daten und bereits berechneten Kursmarken.
- Erfinde, verändere oder verschiebe KEINE Kursmarken, Ziele, Trigger, Indikatoren oder sonstigen technischen Werte.
- Berechne selbst KEINE neuen Kursmarken.
- Der MTF-Score ist eine interne Berechnungsgröße und darf NICHT erwähnt werden.
- Vermische die Zeithorizonte nicht. 1h beschreibt die übergeordnete Intraday-Richtung, 30m das Setup und 15m die kurzfristige Bestätigung bzw. Warnung.
- Beziehe die bereits berechnete Intraday-Chartstruktur ausdrücklich ein. Eine bullische 30m/15m-Struktur innerhalb eines Abwärtskanals ist als bullische Erholung innerhalb dieses Kanals zu beschreiben, nicht als bestätigter Ausbruch.
- Trenne drei Entwicklungsstufen strikt voneinander: (1) der bereits berechnete Szenario-Trigger aktiviert das bestehende Bull-/Bär-Szenario, (2) die aktuell berechnete obere bzw. untere Kanalbegrenzung markiert den strukturellen Kanal-Ausbruch bzw. Kanal-Bruch, und (3) bereits berechnete übergeordnete Widerstands-/Durchschnittsstrukturen liefern die weitere Bestätigung. Klassifiziere JEDE höhere Marke EINZELN anhand ihrer tatsächlichen Lage relativ zur relevanten Kanalgrenze und niemals gesammelt: (a) innerhalb des Kanals, (b) auf bzw. nahe der Kanalgrenze, (c) oberhalb der oberen Kanalgrenze oder (d) unterhalb der unteren Kanalgrenze. Nenne für JEDE relevante Marke ihre eigene Kategorie ausdrücklich in einem eigenen Teilsatz; fasse niemals zwei oder mehr Marken unter einer gemeinsamen Lagebeschreibung zusammen. Schreibe insbesondere NICHT sinngemäß „EMA200 und EMA50 liegen innerhalb des Kanals“, wenn ihre tatsächlichen Lagen unterschiedlich sind. Bei einer oberen Kanalgrenze sind Marken darunter vorgelagerte Hürden, Marken auf bzw. nahe der Grenze grenznahe Hürden und Marken darüber nachgelagerte Bestätigungen; bei einer unteren Kanalgrenze gilt die spiegelbildliche Einordnung. Verwende für diese Klassifizierung ausschließlich die gelieferten Werte und nenne die konkrete Lage jeder relevanten Marke. Ein Szenario-Trigger ist NICHT automatisch ein Kanal-Ausbruch.
- Wenn der Kurs innerhalb eines Abwärtskanals liegt, beschreibe einen Überschritt des bullischen Szenario-Triggers zunächst als Aktivierung des bullischen Szenarios. Einen strukturellen Ausbruch darfst du erst oberhalb der tatsächlich berechneten oberen Kanalbegrenzung formulieren. Befindet sich der Kurs bereits im oberen Kanalbereich, beziehe die berechnete Kanalposition und den Abstand zur oberen Begrenzung in die Einschätzung ein.
- Formuliere einen Kanal-Ausbruch nicht als "Widerlegung" des gesamten bärischen Marktbildes. Ein Kanal-Ausbruch löst zunächst die bestehende Kanalstruktur auf bzw. schwächt sie deutlich; die übergeordnete bullische Bestätigung bleibt an die bereits berechneten darüberliegenden Widerstände und die vorhandene MA-/Positionsstruktur gebunden.
- Die bestehenden Szenario-Marken aus dem Programm sind verbindlich und dürfen nur in der gelieferten Form verwendet werden.
- Die lokalen Daytrading-Trigger sind ebenfalls verbindlich und dürfen nicht durch eigene Marken ersetzt werden.
- Keine Kauf-/Verkaufsempfehlung und keine Behauptung einer sicheren Vorhersage. Formuliere stattdessen konditionale Szenarien.

AUSGABE:
Schreibe eine kompakte Zukunftseinschätzung in GENAU 4 Sätzen, deutsch, sachlich, ohne Anrede. Jeder Satz beginnt exakt mit der folgenden Kennzeichnung:
1. "Intraday:" – Welche kurzfristige Entwicklung wird durch 1h/30m/15m aktuell unterstützt und wie ist sie im Verhältnis zur bereits berechneten Intraday-Chartstruktur einzuordnen?
2. "Bestätigung:" – Stelle die drei Entwicklungsstufen klar getrennt dar: welcher bereits berechnete Szenario-Trigger aktiviert das Szenario, welche bereits berechnete Kanalbegrenzung wäre der strukturelle Ausbruch bzw. Kanal-Bruch und welche bereits berechneten Widerstands-/Durchschnittsstrukturen liefern die weitere Bestätigung. Ordne jede genannte Struktur nach ihrer tatsächlichen Lage zur Kanalgrenze; eine Marke innerhalb des Kanals ist eine vorgelagerte Hürde und keine nachgelagerte Bestätigung. Verwende nur die gelieferten Werte.
...
CHARTSTRUKTUR DES BEREITS ERZEUGTEN INTRADAY-CHARTS:
{(chartstruktur.get("text") if chartstruktur else "Keine Intraday-Chartstruktur verfügbar.")}
Diese Chartstruktur stammt aus derselben bestehenden Berechnung wie die sichtbare Grafik. Für die Zukunftseinschätzung sind die numerischen Kanalgrenzen am gemeinsamen 1h-Kanalbezugszeitpunkt verbindlich:
{chart_quantitative_block}
Eine bullische 30m/15m-Struktur darf NICHT als Ausbruch aus einer übergeordneten bärischen Chartformation bezeichnet werden, wenn der Kurs laut Chartstruktur noch innerhalb dieser Formation liegt. Formuliere in diesem Fall ausdrücklich als Erholung innerhalb der übergeordneten Struktur. Beziehe die Distanz zur oberen bzw. unteren Begrenzung ein, wenn sie für die erwartete nächste Entwicklung relevant ist.
WICHTIGE UNTERSCHEIDUNG: Der bestehende bullische Szenario-Trigger aus dem Pivot-Block ist NICHT automatisch ein struktureller Kanal-Ausbruch. Ein Überschreiten des Szenario-Triggers aktiviert nur das bereits berechnete bullische Szenario. Ein struktureller Ausbruch aus dem Abwärtskanal liegt erst oberhalb der aktuell berechneten oberen Kanalbegrenzung vor. Wenn beide Marken unterschiedlich sind, nenne diese Ebenen ausdrücklich getrennt und verwechsle sie nicht.
Der separate Realtime-Kurs kann aktueller sein als der letzte 1h-Schlusskurs. Für Abstände, Position im Kanal und die Kanalgrenzen darf ausschließlich der oben angegebene Kanalbezugs-Kurs am exakt gleichen Berechnungszeitpunkt verwendet werden. Verwende den Realtime-Kurs nicht für diese Kanalabstandsberechnung.
...
{lokale_daytrading_block}
{szenarien_block}
WICHTIGE MARKEN-EBENEN:
- Der Aufwärts-Trigger aus dem bestehenden Szenario-Block aktiviert das bullische Szenario.
- Die obere Kanalbegrenzung ist davon getrennt und markiert den strukturellen Kanal-Ausbruch.
- Wenn beide Werte verschieden sind, behandle sie als zwei getrennte Stufen; die Reihenfolge weiterer Widerstands-/Durchschnittsstrukturen richtet sich ausschließlich nach deren tatsächlicher Lage zur Kanalgrenze.


MITTELFRISTIGE SZENARIO-ERGEBNISSE AUS DER BEREITS BERECHNETEN STRUKTUR (VERBINDLICH):
...
WICHTIG: Die Zahlen und Marken im Text müssen ausschließlich aus den oben gelieferten, bereits berechneten Werten stammen. Keine eigenen Ableitungen, keine neuen Trigger und keine neuen Ziele."""
...
def analysiere_intraday_chartstruktur(intraday_reihe):
    """Übernimmt exakt die vorhandene Intraday-Chartstruktur in Textform.
    Es wird keine neue Chartlogik eingeführt: Hauptkanal und Zusatzkanal werden
    mit denselben Funktionen/Parametern ermittelt, die baue_chart() bereits
    verwendet. Die Grafik selbst bleibt unverändert.
    """
    if intraday_reihe is None or len(intraday_reihe) < 20:
        return None

    kanal = finde_trendkanal(
        intraday_reihe,
        fenster=INTRADAY_KANAL_FENSTER,
        min_punkte=INTRADAY_KANAL_MIN_PUNKTE,
    )
    close = float(intraday_reihe["Close"].iloc[-1])
    chart_text = None

    kanal_details = None
    if kanal is not None:
        x = mdates.date2num(intraday_reihe.index[-1])
        obere = float(kanal["obere_linie"][0] * x + kanal["obere_linie"][1])
        untere = float(kanal["untere_linie"][0] * x + kanal["untere_linie"][1])
        breite = max(0.0, obere - untere)
        abstand_oben = obere - close
        abstand_unten = close - untere
        if close > obere:
            kurslage = "Kurs oberhalb der oberen Begrenzung"
        elif close < untere:
            kurslage = "Kurs unterhalb der unteren Begrenzung"
        else:
            kurslage = "Kurs innerhalb der Formation"
        position_pct = ((close - untere) / breite * 100.0) if breite > 0 else None
        kanal_details = {
            # Kurs und Kanalgrenzen werden bewusst auf exakt denselben
            # 1h-Kerzenzeitpunkt bezogen. Der separate /price-Livekurs kann
            # aktueller sein, besitzt aber keinen verlässlichen Zeitstempel.
            "kurs": close,
            "kurs_zeitpunkt": intraday_reihe.index[-1],
            "obere_grenze": obere,
            "untere_grenze": untere,
            "kanalbreite": breite,
            "abstand_oben": abstand_oben,
            "abstand_unten": abstand_unten,
            "abstand_oben_pct": (abstand_oben / close * 100.0) if close else None,
            "abstand_unten_pct": (abstand_unten / close * 100.0) if close else None,
            "position_im_kanal_pct": position_pct,
            "kurslage": kurslage,
        }
        if position_pct is None:
            position_text = ""
        elif 0.0 <= position_pct <= 100.0:
            position_text = f" | Position im Kanal {position_pct:.1f}% von unten"
        elif position_pct > 100.0:
            position_text = f" | Position relativ zum Kanal {position_pct:.1f}% (oberhalb)"
        else:
            position_text = f" | Position relativ zum Kanal {position_pct:.1f}% (unterhalb)"
        chart_text = (
            f"{kanal['formation']} | {kurslage} | "
            f"obere Begrenzung {obere:.2f} USD | untere Begrenzung {untere:.2f} USD | "
            f"Abstand oben {abstand_oben:+.2f} USD | Abstand unten {abstand_unten:+.2f} USD"
            f"{position_text}"
        )
    else:
        # Exakt derselbe Fallback wie in baue_chart(): lineare Regression über
        # die zweite Hälfte der Intraday-Reihe.
        preise = intraday_reihe["Close"]
        trend_ausschnitt = preise.iloc[len(preise) // 2:]
        if len(trend_ausschnitt) >= 2:
            x_num = mdates.date2num(trend_ausschnitt.index)
            steigung, achsenabschnitt = np.polyfit(x_num, trend_ausschnitt.values, 1)
            trend_aktuell = float(steigung * x_num[-1] + achsenabschnitt)
            richtung = "Aufwärtstrend" if steigung > 0 else "Abwärtstrend"
            kurslage = "Kurs oberhalb der Trendlinie" if close > trend_aktuell else "Kurs unterhalb der Trendlinie"
            chart_text = f"{richtung} | {kurslage}"

    zusatz = kanal_seit_wendepunkt(intraday_reihe)
    if zusatz is not None:
        start_text = zusatz["start"].strftime("%d.%m. %H:%M")
        if zusatz["typ"] == "kanal":
            zusatz_text = f"{zusatz['daten']['formation']} seit {start_text}"
        else:
            steigung = zusatz["daten"][0]
            zusatz_text = f"{'Aufwärtstrend' if steigung > 0 else 'Abwärtstrend'} seit {start_text}"
        chart_text = f"{chart_text} | Zusatzstruktur: {zusatz_text}" if chart_text else f"Zusatzstruktur: {zusatz_text}"

    if not chart_text:
        return None
    return {
        "formation": kanal["formation"] if kanal is not None else None,
        "text": chart_text,
        "kurs": close,
        "kurs_zeitpunkt": intraday_reihe.index[-1],
        "kanal": kanal_details,
...
def baue_html(daten, pivots, tendenz_label, tendenz_pct, rueckblick_text, chart_dateiname, chart_tages_dateiname, positionstrading_status, range_ausbruch_status, economic_events_block, zonen_je_zeitraum, struktur_6m_daten=None, positionstrading_daten=None, struktur_6m_szenario_zonen=None, struktur_6m_reaktionszonen=None, mittelfristige_szenarien=None, intraday_zukunft=None, tages_ma_struktur=None):
    ...
    <h3 style="color:#a89d87;font-size:12px;letter-spacing:1px;text-transform:uppercase;margin-top:22px;">Zukunftseinschätzung</h3>
...
def main():
    ...
    szenarien = berechne_szenarien(daten["realtime"], pivots)
    # Dieselbe bereits vorhandene Kanal-/Trendstruktur des Intraday-Charts
    # wird zusätzlich der Zukunftseinschätzung bereitgestellt. Die Chartgrafik
    # selbst bleibt unverändert; es wird nur ihr berechneter Strukturkontext geteilt.
    intraday_chartstruktur = analysiere_intraday_chartstruktur(daten["intraday_reihe"])
    intraday_zukunft = analysiere_intraday_zukunft(
        daten, szenarien, intraday_chartstruktur=intraday_chartstruktur
    )
