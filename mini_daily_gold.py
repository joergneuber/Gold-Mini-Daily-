...
{(intraday_zukunft.get("chartstruktur", {}).get("text") if intraday_zukunft and intraday_zukunft.get("chartstruktur") else "Keine Intraday-Chartstruktur verfügbar.")}
Diese Chartstruktur stammt aus derselben bestehenden Berechnung wie die sichtbare Grafik. Eine bullische 30m/15m-Struktur darf NICHT als Ausbruch aus einer übergeordneten bärischen Chartformation bezeichnet werden, wenn der Kurs laut Chartstruktur noch innerhalb dieser Formation liegt. Formuliere in diesem Fall ausdrücklich als Erholung innerhalb der übergeordneten Struktur.
...
def analysiere_intraday_chartstruktur(intraday_reihe):
    ...
    if kanal is not None:
        ...
        chart_text = f"{kanal['formation']} | {kurslage}"
    else:
