...
def finde_intraday_reaktionszonen_baender(
    ...
    Gesucht werden mehrfach getestete Swing-Hochs/-Tiefs. Entscheidend ist NICHT
    eine exakte Marke, sondern dass mindestens `min_treffer` getrennte Reaktionen
    innerhalb eines Preisbereichs von maximal 2 * `zonen_halbbreite_usd` liegen.

    Standard: +/- 7 USD um den Mittelwert der Treffer = 14 USD Gesamtbreite.
    Das entspricht exakt der gewünschten 10-15-USD-Darstellung.
    """
    ...
    def bilde_zonen(punkte):
        ...
        zonen = []
        max_breite = 2.0 * zonen_halbbreite_usd
        ...
        while len(rest) >= min_treffer:
            ...
            for i in range(len(rest)):
                ...
                # Zuerst Anzahl der unabhängigen Reaktionen, dann Kompaktheit,
                # dann Aktualität der letzten Reaktion.
                score = (len(gruppe), -spannweite, max(i for i, _ in gruppe))
...
def baue_chart(intraday_reihe, pivots, strukturzonen=None, range_ausbruch_status=None,
    ...
    for zone in reaktionszonen["widerstandszonen"]:
        zeichne_zone(zone)
    for zone in reaktionszonen["supportzonen"]:
        zeichne_zone(zone)
...
def main():
    ...
    print(f"Intraday-Reaktionszonen (1h): Widerstand: {_zone_log(intraday_reaktionszonen['widerstandszonen'])}")
    print(f"Intraday-Reaktionszonen (1h): Support: {_zone_log(intraday_reaktionszonen['supportzonen'])}")
