...
def finde_intraday_reaktionszonen_baender(
    ...
    """Findet echte Intraday-Reaktionszonen aus mehrfachen Umkehrpunkten.

    Die Logik entspricht bewusst dem gewünschten manuellen Vorgehen:
    "Finde im Intraday-Verlauf Preisbereiche, an denen der Kurs mehrfach
    reagiert bzw. umkehrt, und stelle diese als +/- 5-8-USD-Zone dar."

    Wichtig:
    - Es wird NICHT nach einer exakten Marke gesucht.
    - Mindestens `min_treffer` voneinander getrennte Swing-Reaktionen müssen
      innerhalb derselben Preiszone liegen.
    - Die Standardzone ist +/- 7 USD um den berechneten Zonenmittelpunkt.
      Damit beträgt die gesamte Zone 14 USD und liegt genau im gewünschten
      Bereich +/- 5-8 USD.
    - Hochpunkte bilden Widerstandszonen, Tiefpunkte Supportzonen.
    - Die Reaktionspunkte werden nicht einfach auf einen festen Bucket gerundet.
      Dadurch können z.B. mehrere Reaktionen rund um 4.360 USD gemeinsam eine
      Zone bilden, obwohl sie bei 4.356 / 4.361 / 4.366 liegen.
    - Ein Niveau wie 4.342 USD wird nicht automatisch mit 4.360 USD vermischt:
      zwei Zonen dürfen sich nur dann zusammenfassen, wenn ihre tatsächlichen
      Reaktionspunkte noch innerhalb einer einzigen +/- 7-USD-Zone liegen.
    - Keine Look-ahead-Änderung: Ein Swingpunkt braucht `fenster` Kerzen links
      und rechts zur Bestätigung.

    Diese Funktion verändert ausschließlich die Darstellung der
    Intraday-Reaktionszonen. Pivot-, Range-Ausbruch- und
    Positionstrading-Logik bleiben unangetastet.
    """
    ...
    # Bestätigte lokale Umkehrpunkte.
    swing_highs = []
    swing_lows = []
    for i in range(fenster, n - fenster):
        if high[i] >= high[i - fenster:i].max() and high[i] > high[i + 1:i + fenster + 1].max():
            swing_highs.append((i, float(high[i])))
        if low[i] <= low[i - fenster:i].min() and low[i] < low[i + 1:i + fenster + 1].min():
            swing_lows.append((i, float(low[i])))

    def bilde_zonen(punkte):
        """Cluster nach realer Preisnähe, nicht nach festen Preis-Buckets."""
        if len(punkte) < min_treffer:
            return []

        rest = sorted(punkte, key=lambda x: x[1])
        zonen = []

        # Solange noch Punkte vorhanden sind, suchen wir die dichteste Gruppe,
        # deren gesamte Preisspanne in +/- zonen_halbbreite_usd passt.
        while len(rest) >= min_treffer:
            ...
            for i in range(len(rest)):
                ...
                for j in range(i, len(rest)):
                    if rest[j][1] - rest[i][1] <= 2 * zonen_halbbreite_usd:
                ...
                # Primär Anzahl der Treffer, sekundär die Kompaktheit der Zone.
                spannweite = max(preise) - min(preise)
                score = (len(gruppe), -spannweite)
            ...
            preise = [p for _, p in beste_gruppe]
            mittel = float(np.mean(preise))
            unterkante = mittel - zonen_halbbreite_usd
            oberkante = mittel + zonen_halbbreite_usd

            # Nur die tatsächlich verwendeten Treffer aus dem Rest entfernen.
            ...
            zonen.append({
                ...
                "erster_index": min(i for i, _ in beste_gruppe),
                "letzter_index": max(i for i, _ in beste_gruppe),
            })

        # Wichtig: stärkste Zone zuerst; bei gleicher Trefferzahl die jüngere.
        zonen.sort(key=lambda z: (-z["treffer"], -z["letzter_index"]))
        return zonen[:top_n]
...
def baue_chart(intraday_reihe, pivots, strukturzonen=None, range_ausbruch_status=None, pfad="chart.png"):
    ...
    def zeichne_zone(zone):
        ...
        oberkante = min(zone["oberkante"], y_oben)
