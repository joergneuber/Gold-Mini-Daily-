...
def finde_intraday_umkehrzonen(
    intraday_reihe,
    lookback_stunden=36,
    min_treffer=2,
    min_abstand_stunden=3,
    bucket_usd=5,
    reaktion_usd=2,
    reaktionsfenster_stunden=4,
    top_n=4,
):
    """Ermittelt echte Intraday-Reaktionszonen aus den letzten 36 Stunden.

    Eine Zone braucht mindestens 2 getrennte Berührungen. Zwischen zwei
    gezählten Berührungen müssen mindestens 3 Stunden liegen. Eine Berührung
    zählt nur dann, wenn danach innerhalb von 4 Stunden eine Gegenbewegung von
    mindestens 2 USD erfolgt.

    Widerstand:
      lokales Hoch + anschließender Rückgang >= reaktion_usd.
    Support:
      lokales Tief + anschließender Anstieg >= reaktion_usd.

    Die Preisnähe wird in 5-USD-Buckets zusammengefasst. Die Funktion liefert
    die tatsächliche Reaktionszone (gerundeter Bucketpreis) und die Anzahl der
    bestätigten, zeitlich getrennten Reaktionen.
    """
    if intraday_reihe is None or intraday_reihe.empty:
        return {"widerstandszonen": [], "supportzonen": []}

    daten = intraday_reihe.sort_index().copy()
    cutoff = daten.index[-1] - pd.Timedelta(hours=lookback_stunden)
    daten = daten.loc[daten.index >= cutoff]

    if len(daten) < 2 * 1 + reaktionsfenster_stunden + 1:
        return {"widerstandszonen": [], "supportzonen": []}

    highs = daten["High"].astype(float)
    lows = daten["Low"].astype(float)

    # Lokale Extrempunkte mit nur einer Kerze links/rechts:
    # die anschließende 3h-Trennung verhindert, dass eine einzelne Bewegung
    # mehrfach als "Treffer" gezählt wird.
    swing_highs = []
    swing_lows = []
    fenster = 1

    for i in range(fenster, len(daten) - fenster):
        h = float(highs.iloc[i])
        l = float(lows.iloc[i])

        if h >= float(highs.iloc[i - fenster:i + fenster + 1].max()):
            swing_highs.append((daten.index[i], h))

        if l <= float(lows.iloc[i - fenster:i + fenster + 1].min()):
            swing_lows.append((daten.index[i], l))

    def bestaetige_reaktion(kandidaten, richtung):
        """Filtert Extrempunkte auf eine echte Gegenreaktion."""
        bestaetigt = []
        for zeit, preis in kandidaten:
            pos = daten.index.get_loc(zeit)
            ende = min(len(daten), pos + reaktionsfenster_stunden + 1)

            if pos + 1 >= ende:
                continue

            danach = daten.iloc[pos + 1:ende]
            if richtung == "widerstand":
                gegenbewegung = preis - float(danach["Low"].min())
            else:
                gegenbewegung = float(danach["High"].max()) - preis

            if gegenbewegung >= reaktion_usd:
                bestaetigt.append((zeit, preis))
        return bestaetigt

    bestaetigte_highs = bestaetige_reaktion(swing_highs, "widerstand")
    bestaetigte_lows = bestaetige_reaktion(swing_lows, "support")

    def clustern(kandidaten):
        """Bildet Preiscluster und zählt nur zeitlich getrennte Treffer."""
        buckets = {}
        for zeit, preis in kandidaten:
            key = round(preis / bucket_usd) * bucket_usd
            buckets.setdefault(key, []).append((zeit, preis))
        ...
        zonen = []
        min_abstand = pd.Timedelta(hours=min_abstand_stunden)

        for bucket, treffer in buckets.items():
            treffer.sort(key=lambda x: x[0])

            # Greedy-Auswahl chronologisch: ein neuer Treffer zählt nur,
            # wenn er mindestens 3 Stunden vom zuletzt gezählten entfernt ist.
            ausgewaehlt = []
            for zeit, preis in treffer:
                if not ausgewaehlt or zeit - ausgewaehlt[-1][0] >= min_abstand:
                    ausgewaehlt.append((zeit, preis))

            if len(ausgewaehlt) >= min_treffer:
                zonen.append((float(bucket), len(ausgewaehlt)))

        zonen.sort(key=lambda z: (-z[1], abs(z[0] - float(daten["Close"].iloc[-1]))))
        return zonen[:top_n]

    widerstaende = clustern(bestaetigte_highs)
    supports = clustern(bestaetigte_lows)

    return {
        "widerstandszonen": widerstaende,
        "supportzonen": supports,
    }
...
def baue_chart(intraday_reihe, pivots, strukturzonen=None, range_ausbruch_status=None, pfad="chart.png"):
    ...
    umkehrzonen = finde_intraday_umkehrzonen(intraday_reihe, top_n=4)
    alle_umkehr_preise = (
    ...
    # Reaktionszonen zeichnen: mehrfach bestätigte Intraday-Reaktionen, jede einzeln als Linie -
...
def main():
    ...
    intraday_umkehrzonen = finde_intraday_umkehrzonen(daten["intraday_reihe"], top_n=4)
    print(f"Intraday-Reaktionszonen (36h): Widerstand {intraday_umkehrzonen['widerstandszonen']} | Support {intraday_umkehrzonen['supportzonen']}")
