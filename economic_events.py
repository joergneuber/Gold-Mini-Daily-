...
def _sanitize_fed_events(events):
    """Entfernt falsche/veraltete FOMC-Klassifizierungen aus Live-/Cache-Daten.

    Die Funktion bleibt als bewusst kleine Kompatibilitätsschicht erhalten:
    Die neue Event-Pipeline normalisiert weiterhin alle Quellen gemeinsam,
    während FOMC-Daten zusätzlich gegen die für 2026 bekannten offiziellen
    Sitzungstage geprüft werden.
    """
    if datetime.now(TZ_DE).year != 2026:
        return events
    valid_decisions = {
    ...
    for event in events:
        ...
        event_date = str(event.get("date", event.get("datetime", "")))[:10]
        canonical = str(event.get("canonical", ""))
        if name == "FOMC / Fed-Zinsentscheid" or canonical == "FOMC":
            # Die Minutes werden separat anhand ihres eigenen Namens geprüft.
            if name == "FOMC Minutes (Juli-Sitzung)":
                if event_date not in valid_minutes:
                    continue
            elif event_date not in valid_decisions:
                continue
        elif name == "FOMC Minutes (Juli-Sitzung)":
            if event_date not in valid_minutes:
                continue
        out.append(event)
    return out
